...
class SortedLongDocValuesType implements FieldTypeInterface {

	SortedLongDocValuesType() {
		super();
	}
	...
	final public void fillDocument(final String fieldName, final Object value, Document doc) {
		if (value instanceof Collection)
			...
			doc.add(new SortedNumericDocValuesField(fieldName, ((Number) value).longValue()));
		else
			doc.add(new SortedNumericDocValuesField(fieldName, Long.parseLong(value.toString())));
	}
