...
public class DoublePrimitiveArraySetterTest extends AbstractMultipleSetterTest {

	double[] value;

	public DoublePrimitiveArraySetterTest() throws NoSuchFieldException {
		super(new DoublePrimitiveArraySetterImpl(DoublePrimitiveArraySetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... next) {
		...
		int i = 0;
		for (String v : next)
			Assert.assertEquals(Double.parseDouble(v), value[i++], 0);
	}
	...
	protected void checkValueNumber(Number... next) {
		...
		int i = 0;
		for (Number v : next)
			Assert.assertEquals(v.doubleValue(), value[i++], 0);
	}
	...
	protected void checkValueChar(Character... next) {
		...
		for (Character v : next)
			Assert.assertEquals((double) v, value[i++], 0);
	}
