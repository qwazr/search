...
public class FloatArraySetterTest extends AbstractArraySetterTest<Float> {
	...
	public FloatArraySetterTest() throws NoSuchFieldException {
		super(new FloatArraySetterImpl(AbstractArraySetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... values) {
		...
		int i = 0;
		for (String v : values)
			Assert.assertEquals(Float.parseFloat(v), get(i++), 0);
	}
	...
	protected void checkValueNumber(Number... values) {
		...
		int i = 0;
		for (Number v : values)
			Assert.assertEquals(v.floatValue(), get(i++), 0);
	}
