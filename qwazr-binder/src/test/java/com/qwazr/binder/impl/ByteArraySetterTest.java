...
public class ByteArraySetterTest extends AbstractArraySetterTest<Byte> {

	public ByteArraySetterTest() throws NoSuchFieldException {
		super(new ByteArraySetterImpl(AbstractArraySetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... values) {
		...
		int i = 0;
		for (String v : values)
			Assert.assertEquals(Byte.parseByte(v), get(i++), 0);
	}
	...
	protected void checkValueNumber(Number... values) {
		...
		int i = 0;
		for (Number v : values)
			Assert.assertEquals(v.byteValue(), get(i++), 0);
	}
