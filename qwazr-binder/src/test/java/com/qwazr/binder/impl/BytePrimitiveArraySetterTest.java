...
public class BytePrimitiveArraySetterTest extends AbstractMultipleSetterTest {

	byte[] value;

	public BytePrimitiveArraySetterTest() throws NoSuchFieldException {
		super(new BytePrimitiveArraySetterImpl(BytePrimitiveArraySetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... next) {
		...
		int i = 0;
		for (String v : next)
			Assert.assertEquals(Byte.parseByte(v), value[i++], 0);
	}
	...
	protected void checkValueNumber(Number... next) {
		...
		int i = 0;
		for (Number v : next)
			Assert.assertEquals(v.byteValue(), value[i++], 0);
	}
	...
	protected void checkValueChar(Character... next) {
		...
		for (Character v : next)
			Assert.assertEquals((byte) v.charValue(), value[i++], 0);
	}
	...
	protected void checkValueObject(Object... values) {
		...
		int i = 0;
		for (byte v : value)
			Assert.assertEquals(v, (byte) values[i++], 0);
	}
}
