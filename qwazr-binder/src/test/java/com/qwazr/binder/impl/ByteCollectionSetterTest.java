...
public class ByteCollectionSetterTest extends AbstractCollectionSetterTest<Byte> {
	...
	public ByteCollectionSetterTest() throws NoSuchFieldException {
		super(new ByteCollectionSetterImpl(AbstractCollectionSetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... values) {
		...
		int i = 0;
		for (Byte v : value)
			Assert.assertEquals(v, Byte.parseByte(values[i++]), 0);
	}
	...
	protected void checkValueNumber(Number... values) {
		...
		int i = 0;
		for (Byte v : value)
			Assert.assertEquals(v, values[i++].byteValue(), 0);
	}
