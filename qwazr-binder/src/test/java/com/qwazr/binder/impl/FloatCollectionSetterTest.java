...
public class FloatCollectionSetterTest extends AbstractCollectionSetterTest<Float> {

	public FloatCollectionSetterTest() throws NoSuchFieldException {
		super(new FloatCollectionSetterImpl(AbstractCollectionSetterTest.class.getDeclaredField("value")));
	}
	...
	protected void checkValueString(String... values) {
		...
		int i = 0;
		for (Float v : value)
			Assert.assertEquals(v, Float.parseFloat(values[i++]), 0);
	}
	...
	protected void checkValueNumber(Number... values) {
		...
		int i = 0;
		for (Float v : value)
			Assert.assertEquals(v, values[i++].floatValue(), 0);
	}
