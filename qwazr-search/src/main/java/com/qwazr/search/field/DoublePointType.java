...
final class DoublePointType extends CustomFieldTypeAbstract {

    private DoublePointType(final Builder<CustomFieldDefinition> builder) {
        super(builder);
    }

    static DoublePointType of(final String genericFieldName,
                              final WildcardMatcher wildcardMatcher,
                              final CustomFieldDefinition definition) {
        return new DoublePointType(CustomFieldTypeAbstract
            .of(genericFieldName, wildcardMatcher, definition)
            .bytesRefConverter(BytesRefUtils.Converter.DOUBLE_POINT)
    ...
    private static FieldSupplier buildFieldSupplier(final CustomFieldDefinition definition) {
        if (isStored(definition))
            return (fieldName, value, documentBuilder) -> {
                ...
                documentBuilder.acceptField(new DoublePoint(fieldName, doubleValue));
                documentBuilder.acceptField(new StoredField(fieldName, doubleValue));
            };
        else
